...
NodePtr Parser::buildTree(const std::vector<std::string> &tokens)
{
    ...
    for (size_t i = 0; i < tokens.size(); ++i)
    {
        ...
        else if (token == "+" || token == "-" || token == "*" || token == "/")
        {
            ...
            {
                throw std::runtime_error("Incorrect statement: There are not enough operands. ");
                ;
            }
            ...
            {
                throw std::runtime_error("Incorrect statement: Unexpected token: "+ tokens[i]);
                ;
            }
        ...
        else if (token == "(")
        {
            ...
            if (balance != 0)
            {
                throw std::runtime_error("Incorrect statement: The parentheses are not balanced.");
                ;
            }
        ...
        else if (token == "sin" || token == "cos" || token == "tan" || token == "cot")
        {
            ...
            {
                throw std::runtime_error("Incorrect statement: Parenthesis is missing for trigonometric function.");
            }
        ...
        else if (token == "ln" || token == "log")
        {
            if (i == tokens.size() - 1)
            {
                throw std::runtime_error("Incorrect statement: The operand for the logarithmic function is missing.");
            }
        ...
        else if (token == "sqrt")
        {
            if (i == tokens.size() - 1)
            {
                throw std::runtime_error("Incorrect statement: The operand for the square root function is missing.");
                ;
            }
        ...
        else if (token == "sinh" || token == "cosh" || token == "tanh" || token == "coth" || token == "sech" || token == "csch")
        {
            if (i == tokens.size() - 1 || tokens[i + 1] != "(")
            {
                throw std::runtime_error("Incorrect statement: Parenthesis missing for hyperbolic function.");
            }
    ...
    if (nodeStack.size() != 1)
    {
        throw std::runtime_error("Incorrect statement: Not enough operators.");
    }
